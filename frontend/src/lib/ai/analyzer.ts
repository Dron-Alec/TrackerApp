...
function buildMockAnalysis(event: Event): AnalysisResult {
  ...
  return {
    ...
    serviceLineInsights: {
      ...
      bsaAml: isCompliance
        ? `${event.name} draws compliance officers from exchanges, custodians, and fintechs who are actively building or maturing their AML programs. FinCEN's updated VASP guidance and the FATF travel rule create immediate demand for Nexus Advisory's BSA program design and transaction monitoring advisory work.`
        : `Attendees operating in financial services verticals likely face AML program requirements. Nexus Advisory can engage compliance leads on BSA readiness, transaction monitoring system selection, and independent AML testing — especially for firms scaling into new jurisdictions.`,
      riskConsulting: `Enterprise risk management is a consistent gap for fast-growing crypto firms at events like ${event.name}. Nexus Advisory can engage CROs and risk leads on internal controls design, SOC examination readiness, and operational risk frameworks — particularly for custody providers and exchanges with institutional client bases.`,
      advisory: `${event.name} is an ideal venue for Nexus Advisory's advisory practice to connect with firms evaluating regulatory strategy, digital asset product launch planning, or organizational readiness for increased regulatory scrutiny. Strategic advisory engagements often begin with conversations at conferences like this one.`,
    },
    personas: [
      {
        title: 'Chief Compliance Officer',
        description: 'Responsible for the firm\'s entire compliance program — AML, sanctions, licensing. Faces pressure from regulators and institutional investors demanding program maturity. A direct buyer of BSA/AML and advisory services.',
      ...
      {
        title: 'Head of Risk',
        description: 'Builds and owns the enterprise risk framework. Faces growing board pressure to demonstrate robust controls, especially post-FTX. Key buyer for risk consulting and SOC examination services.',
    ...
    keyTakeaway: `${event.name} offers direct access to the compliance, risk, and institutional finance decision-makers who are Nexus Advisory's primary buyers in the digital asset space, making it one of the highest-ROI events for BD investment this year.`,
  };
}
